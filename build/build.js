...
function createControls() {
  ...
  return {
    addEventListener: function addEventListener(id, cb) {
      listeners[id] = cb;
      //listeners.set(id, cb);
    },

    removeEventListener: function removeEventListener(id) {
      delete listeners[id];
      //listeners.delete(id);
    },
...
function buildAxes(length) {
  ...
  axes.add(buildAxis(new THREE.Vector3(0, 0, 0), new THREE.Vector3(length, 0, 0), 16711680, false)); // +X red
  axes.add(buildAxis(new THREE.Vector3(0, 0, 0), new THREE.Vector3(-length, 0, 0), 16711680, true)); // -X red dashed
  axes.add(buildAxis(new THREE.Vector3(0, 0, 0), new THREE.Vector3(0, length, 0), 65280, false)); // +Y green
  axes.add(buildAxis(new THREE.Vector3(0, 0, 0), new THREE.Vector3(0, -length, 0), 65280, true)); // -Y green dashed
  axes.add(buildAxis(new THREE.Vector3(0, 0, 0), new THREE.Vector3(0, 0, length), 255, false)); // +Z blue
  axes.add(buildAxis(new THREE.Vector3(0, 0, 0), new THREE.Vector3(0, 0, -length), 255, true)); // -Z blue dashed
...
var scene3d = _interopRequire(require("./scene3d"));
...
},{"./scene3d":5,"babelify/polyfill":10}],5:[function(require,module,exports){
"use strict";

var _interopRequire = function (obj) { return obj && obj.__esModule ? obj["default"] : obj; };

var createDebugAxes = _interopRequire(require("./debug_axes_3d"));

var createDebugCube = _interopRequire(require("./debug_cube"));

var createControls = _interopRequire(require("./create_controls"));
...
function init() {
  ...
  world = new THREE.Mesh(new THREE.PlaneGeometry(200, 200, 20, 20), new THREE.MeshBasicMaterial({ wireframe: true, color: 0 }));
  world.position.z = 50;
  pivot.add(world);

  var light = new THREE.HemisphereLight(7829367, 0, 0.6);
  scene.add(light);
...
},{"./create_controls":1,"./debug_axes_3d":2,"./debug_cube":3}],6:[function(require,module,exports){
(function (global){
"use strict";
